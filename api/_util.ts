...
/** Normalize various inputs to RAW initData string */
...
/** Build data_check_string by Telegram rules: all keys except 'hash', sorted by key, raw (not decoded) */
function buildDataCheckString(rawInitData: string): { dcs: string; providedHash: string } {
  ...
  for (const p of parts) {
    ...
    if (i <= 0) continue;
    const k = p.slice(0, i);
    if (k === "hash") continue;         // EXCLUDE only 'hash'
